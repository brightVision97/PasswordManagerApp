...
public class CryptoUtilsImpl implements CryptoUtils
{
    ...
    {
        KeySpec keySpec = new PBEKeySpec(key.toCharArray(), salt, Constants.ITERATION_COUNT);
        SecretKey secretKey = SecretKeyFactory.getInstance(Constants.ENCRYPTING_ALGORITHM)
                .generateSecret(keySpec);
        
        AlgorithmParameterSpec paramSpec = new PBEParameterSpec(salt, Constants.ITERATION_COUNT);
            ...
            IllegalBlockSizeException
    {
        ...
        Cipher decryptCipher = Cipher.getInstance(secretKey.getAlgorithm());
